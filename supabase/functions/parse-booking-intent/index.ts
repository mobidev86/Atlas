...
Deno.serve(async req => {
  ...
  try {
     ...
     * -----------------------------------------
     *
     * Asia/Kolkata is used for resolving
     * relative dates.
     */
    ...
    console.log('--------------------------------');
    console.log('PARSE BOOKING INTENT REQUEST');
     ...
     */
    const systemPrompt = `
You are a travel booking intent parser.
...
The application supports ONLY ONE booking type at a time.
...
`;
     ...
     * -----------------------------------------
     */
    let bookingIntent: any;

    try {
      bookingIntent = JSON.parse(outputText);
    } catch {
    ...
    console.log('Booking intent:', JSON.stringify(bookingIntent, null, 2));
     ...
     */
    if (
      bookingIntent?.type !== 'hotel' &&
      bookingIntent?.type !== 'flight' &&
      bookingIntent?.type !== 'unsupported'
    ) {
      throw new Error('Invalid booking intent type returned by OpenAI');
    }
     ...
     * -----------------------------------------
     *
     * Return the parsed intent to the app.
     * This allows the app to decide what to show.
     */
    if (bookingIntent?.type === 'unsupported') {
      return new Response(
        JSON.stringify({
          success: true,
